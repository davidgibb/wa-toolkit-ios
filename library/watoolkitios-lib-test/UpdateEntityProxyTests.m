...
#import "UpdateEntityProxyTests.h"
#import "WAToolkit.h"

@implementation UpdateEntityProxyTests
...
-(void)testShouldUpdateTableEntityWithCompletionHandler
{
    ...
    [proxyClient updateEntity:_testEntity withCompletionHandler:^(NSError *error) {
    ...
    [proxyClient fetchEntitiesWithRequest:fetchRequest usingCompletionHandler:^(NSArray *entities, WAResultContinuation *resultContinuation, NSError *error) {
        ...
        XCTAssertEqualObjects([entityFound objectForKey:@"Price"], @"299", @"Entity was not updated.");
        [proxyDelegate markAsComplete];
    }];
    [proxyDelegate waitForResponse];
}
